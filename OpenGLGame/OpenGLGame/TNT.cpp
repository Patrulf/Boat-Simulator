...
TNT::TNT(float p_x, float p_y, float p_z, GLuint p_programID)
{
	...
	mat4 scaleMatrix = scale(scale_);
	modelMatrix = translationMatrix * rotationMatrix * scaleMatrix;

	sphere = new Sphere(1.0f);
	sphere->GenerateVerticesAndIndices(1, 0, 0);
...
void TNT::Update(float p_delta)
{
	...
	circleCollider->SetRadius(scale_);

	scale_ < 2.0f ? scale_ += 1.0f*p_delta : Destruction();
...
void TNT::SetDestructionCallback(std::function<void(TNT*)> p_callback)
{
	destructionCallback = p_callback;
}

void TNT::Destruction()
{
	if (destructionCallback != nullptr)
		destructionCallback(this);
}

CircleCollider* TNT::GetCircleCollider()
{
...
void TNT::SetMatrix(mat4 p_myMatrix)
{
	myMatrix = p_myMatrix;
}